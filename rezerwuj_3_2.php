        ...
        <main>
            <div class="index-content">
                ...
                <?php
                    ...
                    $godzina = substr($termin, 10); // pobranie danych
                
                    $sql = "INSERT INTO `rezerwacje` (`id_pacjenta`, `data`, `godzina`, `id_lekarza`) 
                            VALUES ('$id_pacjenta', '$data', '$godzina', '$id_lekarza')"; // pobranie danych z bazy danych
                    if (mysqli_query($connect, $sql)) {
                        $sql1="update `terminarz` set `dostepnosc`=0 where `dostepnosc`=1 and `id_lekarza`='".$id_lekarza."' and `godzina`='".$_SESSION['godzina']."';";
                        $query1=mysqli_query($connect,$sql1); // zaktualizowanie danych w bazie danych
